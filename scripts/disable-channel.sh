#!/usr/bin/env bash
# disable-channel.sh — Disable a channel
# Usage: disable-channel.sh <channel-name>
...
if [[ -z "${1:-}" ]]; then
  echo "Usage: disable-channel.sh <channel-name>" >&2
  exit 1
fi
...
jq --arg name "$NAME" '
  .channels |= map(if .name == $name then .enabled = false else . end)
' "$CHANNELS_FILE" > "$TMP" && mv "$TMP" "$CHANNELS_FILE"

echo "Channel '$NAME' disabled."
